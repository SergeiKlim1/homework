...
def sort_by_date(transactions: List[Dict[str, Any]], type_sort: Bool = True) -> List[Dict[str, Any]]:
    """"
    Функция, которая принимает список словарей и необязательный параметр,
    задающий порядок сортировки (по умолчанию — убывание) и возвращает
    новый список отсортированный по дате
    """
    sorted_list_by_date = sorted(transactions, key=lambda x: datetime.strptime(x["date"], '%Y-%m-%dT%H:%M:%S.%f'), reverse=type_sort)
